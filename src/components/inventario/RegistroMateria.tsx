...
export default function RegistroMateria() {
    ...
    const onRowEditComplete = (e: DataTableRowEditCompleteEvent) => {
        ...
        _productos[index] = {
            ...newData,
            metroCR: ((((((newData.diametroUno + newData.diametroDos)/2)*((newData.diametroUno + newData.diametroDos)/2)) * newData.largo * 0.7854) / 10).toFixed(2)),
            };
    ...
    const agregarProducto = async() => {
        if (value1 === null || value2 === null || value3 === null || !selectedCalidad) {
            toast.current?.show({
                ...
                detail: 'Por favor, complete todos los campos antes de registrar la materia prima.',
                life: 3000,
            });
  ...
  const registrarProductos = async () => {
    ...
    const productosARegistrar = productosRegistrados.map(producto => ({
        ...
        metroCR: Number((( (((producto.diametroUno + producto.diametroDos)/2)*((producto.diametroUno + producto.diametroDos)/2)) * producto.largo * 0.7854) / 10).toFixed(2)),
    ...
    try {
        const response = await axios.post("https://api.uniecosanmateo.icu/api/rawMaterials", 
...
const registrarInventario = async (productosGuardados: ProductoGuardado[]) => {
    ...
    console.log("Enviando la materia prima al inventario:", inventariosARegistrar);

    try {
        const response = await axios.post("https://api.uniecosanmateo.icu/api/rawMaterialInventory", 
        ...
        if (response.status === 201) {
            ...
            toast.current?.show({
                severity: "success",
                summary: "Registro exitoso",
                detail: "El inventario de materia prima se ha registrado satisfactoriamente.",
                life: 3000,
            });
        ...
        <>
            ...
            <div className="card flex flex-wrap gap-3 p-fluid mt-2">
                <div className="flex-auto">
                    <label htmlFor="stacked-buttons" className="font-bold block mb-2">Diametro Uno</label>
                ...
                <div className="flex-auto">
                    <label htmlFor="minmax-buttons" className="font-bold block mb-2">Diametro Dos</label>
                    <div className="p-inputgroup flex-1">
                        <span className="p-inputgroup-addon">
            ...
            <div className="card flex flex-wrap gap-3 p-fluid">
                <div className="flex-auto">
                    <label htmlFor="stacked-buttons" className="font-bold block mb-2">Calidad de la materia</label>
                    <div className="p-inputgroup flex-1">
                        <span className="p-inputgroup-addon">
            ...
            header="Materia prima registrada">                    
                ...
                />           
            <Column header="Metros CR"
            body={(rowData) => {
                // Realiza el cálculo
                const metroCR = (( (((rowData.diametroUno + rowData.diametroDos)/2)*((rowData.diametroUno + rowData.diametroDos)/2)) * rowData.largo * 0.7854) / 10);
