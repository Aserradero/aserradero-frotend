...
export default function ProductosPrecio() {
 ...
 // Función para actualizar los datos del producto
   const handleSave = async (e?: React.FormEvent) => {
    e?.preventDefault?.();
    if (!selectedProduct || selectedProduct.precioUnitario === 0 || selectedProduct.stockIdealPT === 0) {
      toast.current?.show({
        severity: "warn",
    ...
    try {
      ...
      const response = await axios.put(`https://api.uniecosanmateo.icu/api/productInventory/${selectedProduct.id}`,
        {
 ...
 // Función para mostrar la ventana de confirmación antes de eliminar el producto
    const confirmEliminar = (rowData: any) => {
      confirmDialog({
        ...
        acceptLabel: "Sí",
        rejectLabel: "No",
        accept: () => eliminarProducto(rowData.id,rowData.idProducto), // Si el usuario acepta, eliminar el producto
        reject: () => {
          toast.current?.show({
            severity: "info",
            summary: "Cancelado",
    ...
    const eliminarProducto = async (id: number, idProducto: number) => {
      ...
      try {
        ...
        const response = await axios.delete(`https://api.uniecosanmateo.icu/api/productInventory/${id}`);
    ...
    const eliminarProducto2 = async (id: number) => {
      try {
        // Eliminar el producto mediante una solicitud DELETE
        const response = await axios.delete(`https://api.uniecosanmateo.icu/api/product/${id}`);
        // Actualizar el estado filtrando el producto eliminado
        setProducts1(products1.filter((product) => product.id !== id));
                        ...
                        <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                          <div className="col-span-2 lg:col-span-1">
                            ...
                            <InputNumber 
                              ...
                              onValueChange={(e) =>setSelectedProduct({...selectedProduct!, precioUnitario: e.value ?? 0})}            
                              min={1} 
                              max={1000}
                              minFractionDigits={2} 
                              maxFractionDigits={2}                               
                          ...
                          <div className="col-span-2 lg:col-span-1">
                            ...
                            <InputNumber 
                              ...
                              onValueChange={(e) =>setSelectedProduct({...selectedProduct!, stockIdealPT: e.value ?? 0})} 
                              min={1} 
                              max={50000}
                              minFractionDigits={2} 
                              maxFractionDigits={2}
                          ...
                          <div className="col-span-2 lg:col-span-1">
                            <Label>Grosor</Label>
                            <InputNumber value={selectedProduct?.product.grosor} readOnly />
                          </div>
                          <div className="col-span-2 lg:col-span-1">
                            <Label>Largo</Label>
                            <InputNumber value={selectedProduct?.product.largo} readOnly />
                          </div>
                          <div className="col-span-2 lg:col-span-1">
                            <Label>PiesTabla</Label>
                            <InputNumber value={selectedProduct?.product.piesTabla} readOnly/>
                          </div>
                        </div>
                      </div>
